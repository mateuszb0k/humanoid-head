...
\begin{itemize}
\item Face Detection - A pre-trained BlazeFace \cite{blazeface} model (developed by Google) will be used for fast and efficient face localization (determination of bounding boxes) in image frames. This model is characterized by high performance, which is critical in real-time operating systems.
\item Face Alignment / Landmarking - The MediaPipe \cite{mediapipe}framework (Face Mesh / Face Landmarks module) will be applied. This solution was chosen due to its high precision, optimization for real-time operation, and significantly greater reliability compared to classical, custom-built vision algorithms.
\item User Identification (Face Recognition) - The identity verification process will be implemented using deep neural networks. An aligned (cropped based on facial landmarks) face image will be processed by the model to generate a feature vector (embedding). Subsequently, user recognition will be performed by computing the cosine similarity between the generated vector and reference vectors stored in the system's database.
\end{itemize}
...
\section{Images generated with Flux.1}
To expand our dataset with additional images, we used generative artificial intelligence models. Our choice fell on the advanced Flux.1 [dev] \cite{flux} model, featuring 12 billion parameters. An attempt to run it in the Kaggle Notebook environment proved suboptimal — generating a single image took approximately 5 minutes. For this reason, we conducted the initial model tests through the Hugging Face platform.
...
Sample images generated using the Flux.1 [dev] model are shown in Figure \ref{fig:generated_em}.
...
\begin{figure}[H]
    ...
    \caption{Examples of generated emotions with Flux.1 [dev].}
...
The \texttt{second\_model} emerged as the most successful architecture in our entire study. It achieved an impressive validation accuracy of \textbf{71.04\%}. As shown in its confusion matrix (Figure \ref{fig:cm_second_third} left), it performs exceptionally well in recognizing the "happy" emotion (over 2000 correct predictions), though it still faces challenges distinguishing between "sad" and "neutral" states. The \texttt{third\_model} also provided solid results (\textbf{67.81\%} accuracy), but exhibited slightly more instability during the validation phases.
...
Benchmarking was conducted to quantify the performance delta between the two implementations. To ensure a comprehensive evaluation, the tests were executed across different hardware configurations, comparing legacy CPU-bound methods with modern GPU acceleration.

\begin{itemize}
    \item \textbf{Setup A (Main Workstation):}
    \begin{itemize}
        \item CPU: Intel Core i5-10400F (2.90 GHz, 6 Cores / 12 Threads)
        \item GPU: NVIDIA GeForce GTX 1060 (6GB GDDR5)
    \end{itemize}
    
    \item \textbf{Setup B (Mobile Workstation):}
    \begin{itemize}
        \item CPU: 12th Gen Intel Core i5-12450H (2.00 GHz, 8 Cores / 12 Threads)
        \item GPU: NVIDIA GeForce RTX 3050Ti (4GB GDDR6)
    \end{itemize}

    \item \textbf{Setup C (Target Platform - Raspberry Pi):}
    \begin{itemize}
        \item CPU: Broadcom BCM2711, Quad core Cortex-A72 (ARM v8) 64-bit SoC
        \item GPU: VideoCore VI (Not used for acceleration in this test)
        \item RAM: 8GB LPDDR4
    \end{itemize}
\end{itemize}

The results of the real-time processing performance, measured in Frames Per Second (FPS), are summarized in Table \ref{tab:fps_comparison}. The benchmarking reveals that while CPU-based HOG detection is usable for background tasks (without face presence), it is insufficient for real-time interaction.

\begin{table}[H]
    \centering
    \begin{tabular}{|l|c|c|c|}
        \hline
        \textbf{Hardware Setup} & \textbf{HOG / dlib (CPU)} & \textbf{InsightFace (GPU)} & \textbf{No face detected*} \\
        \hline
        Setup A (i5-10400F / GTX 1060) & 2-3 FPS & \textbf{22-26 FPS} & 35-45 FPS \\
        Setup B (i5-12450H / 3050Ti) & 3-4 FPS & \textbf{20-25 FPS} & 35-45 FPS \\
        Setup C (Raspberry Pi 4) & N/A FPS & N/A & N/A FPS \\
        \hline
    \end{tabular}
    \caption{Performance comparison. *FPS increases significantly when no face is present due to lower computational load on the embedding and emotion models.}
    \label{tab:fps_comparison}
\end{table}

\subsection{Future Architecture: Event-Based Processing}
Following the successful validation of the GPU-accelerated pipeline (Milestone 1), the system architecture will transition from continuous streaming transmission to an event-based model. This shift is designed to optimize resource usage and streamline data flow to the conversation processing unit. 

Instead of analyzing every single frame continuously, face recognition and deep emotion detection will be triggered only by specific threshold events—such as a user entering the camera's field of view or a sudden, mathematically significant shift in facial landmarks. This paradigm will allow the robotic head to conserve computational power, reduce thermal load, and focus strictly on relevant interaction moments.
\section{Conclusion}
