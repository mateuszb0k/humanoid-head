...
\textbf{Majority Voting Filter for Stability} \\
A common issue in single-shot recognition systems is the flickering of identity predictions due to minor lighting changes or head movements. To solve this without adding heavy computational load, we implemented a majority voting mechanism. The system stores the raw recognition results from the last 7 frames in a fast rolling buffer. The final identity is determined by the most common result in this buffer. This ensures that the robot's tracking and JSON telemetry remain perfectly stable, ignoring split-second misclassifications.

\textbf{Event-Driven Emotion Analysis} \\
Running the deep Keras emotion classification model on every single frame caused severe thermal throttling and dropped the system to unplayable framerates. To fix this, the architecture was optimized to run the emotion network strictly on an event-driven basis. The heavy emotion model is only triggered when the majority voting filter confirms a new identity has entered the frame, or when a completely new session starts. Once the emotion is calculated, it is locked and broadcasted in the JSON payload alongside the live bounding box coordinates. This logic allows the system to easily maintain a smooth 14 FPS.

\textbf{Automated Database Management} \\
To comply with privacy constraints and prevent memory bloat, the system features a self-cleaning SQLite database. It stores only numeric feature embeddings and names, ensuring no actual photographs are saved to the disk. An automated cleanup routine is triggered before any new user is saved, enforcing a strict 7-day Time-To-Live (TTL) for all records and a hard cap of 200 profiles. If the limit is exceeded, the oldest records are automatically purged.
