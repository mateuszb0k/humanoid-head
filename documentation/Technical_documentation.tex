...
\section{Hardware Control and Concurrency}
The program running on the microcomputer integrates image analysis with direct control over the mechanical movements of the head. A key element of the software is a dictionary containing predefined movement sequences associated with specific emotional states. Each emotion recognized by the neural network is automatically translated into an appropriate set of positions for the individual servo motors responsible for the artificial face's expressions. Additionally, the system implements smooth eye tracking, where the coordinates of the detected face center are mathematically scaled to corresponding angular values, allowing the robot to maintain eye contact with the human.

To ensure full operational fluidity and avoid freezing the camera image during movements, the software utilizes a multithreaded architecture using the threading library. Frame processing, eye movement control, mouth animation during speech, and listening for terminal commands run in separate, independent background threads. Communication between the visual analysis layer and the movement algorithm takes place via a safe event queue, meaning that detecting a new emotion or a change in speech status immediately and without delay triggers the corresponding mechanical response.
