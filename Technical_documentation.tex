...
The comparative analysis of all tested architectures is compiled in Table \ref{tab:final_results}. The VGG architectures demonstrated remarkable stability, with VGG19 leading the pre-trained models at 70.07\% accuracy. However, the Xception and InceptionV3 architectures struggled with our specific dataset, likely due to the upscaled input resolution and the synthetic nature of the colorization process.
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|}
        ...
        \hline
        second\_model & \textbf{71.04\%} & \textbf{70.19\%} \\
        VGG19 & 70.07\% & 69.47\% \\
        VGG16 & 68.24\% & 66.57\% \\
...
Remarkably, our custom-built \texttt{second\_model} outperformed all heavy, reference DCNN architectures. The Xception and ResNet models, despite their tremendous depth, exhibited slight overfitting tendencies in the late epochs, suggesting their parameter capacity might be too vast for this specific classification task.
...
The comparative study between custom-designed Convolutional Neural Networks and industry-standard transfer learning models (VGG, ResNet, Inception) yielded unexpected but highly satisfactory results. Our custom \texttt{second\_model} achieved the highest classification accuracy at 71.04\%, closely approaching our initial 75\% target. The application of Fine-Tuning and innovative data colorization techniques proved effective, though heavy architectures like ResNet101 and InceptionV3 did not justify their computational overhead in this specific use case. 

The trained \texttt{second\_model} is lightweight enough to be deployed on an edge computing device (Raspberry Pi) and accurate enough to feed reliable affective data to the robot's speech synthesis subsystem, paving the way for empathetic, human-robot interaction.
