...
def get_teacher_room(teacher_name: str, db_filepath="./teachers_info.json"):
    """
    Searches for a staff member in the database and returns their assigned office.

    Args:
        The full name of the person to look up.
        Path to the staff JSON database.

    Returns:
        A dictionary containing the keys 'teacher_name', 'room', and 'building'.
    """
    empty_result = {
        "teacher_name": None,
        "room": None,
...
def search_teacher(teacher_name: str, db_filepath="./teachers_info.json",top_n = 3):
    """
    Returns a list of the most probable staff matches based on the queried name.

    Utilizes a fuzzy string matching algorithm (RapidFuzz) to mitigate transcription 
    errors or misspellings generated by the STT model, retrieving the best possible matches.
    """
    teacher_dict = {}
    if not os.path.exists(db_filepath):
        print( f"Error: Database file '{db_filepath}' not found.")
