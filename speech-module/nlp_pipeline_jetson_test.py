...
def preprocess_stt(text: str) -> str:
    """
    Pre-processes and normalizes the raw text transcribed by the Speech-to-Text (Whisper) model.

    Args:
        text (str): The raw text string generated by the Whisper STT.

    Returns:
        str: The cleaned and normalized text ready for GLiNER analysis.
    """
...
class NlpModel:
    """
    The main asynchronous Natural Language Processing pipeline for the Humanoid-Head.

    This class orchestrates the complete conversation loop: speech recognition (STT),
    intent detection, entity extraction (GLiNER), response generation by
    local LLM, and asynchronous speech synthesis (Piper TTS). It also maintains 
    the conversation history buffer, user identity, and real-time emotional state.
    """
...
def handle_data():
    """
    REST API endpoint that receives telemetry data from the vision module.

    Dynamically updates the user's face visibility status, identity (name), 
