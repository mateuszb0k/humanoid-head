...
from tfboard import get_confusion_matrix, plot_confusion_matrix
import json
...
for gpu in gpus:
    tf.config.experimental.set_memory_growth(gpu,
                                             True)  # prevent TensorFlow from immediatly allocating all available VRAM on detected GPUs
...
train_data = train_data.map(normalize).prefetch(tf.data.AUTOTUNE)
valid_data = valid_data.map(normalize).prefetch(tf.data.AUTOTUNE)
test_data = test_data.map(normalize).prefetch(tf.data.AUTOTUNE)
...
with tf.summary.create_file_writer("all_logs/hparam_tuning").as_default():
...
def second_model():
    ...
    conv2 = layers.Conv2D(32, (3, 3), padding='same', use_bias=False)(pool1)
    conv2 = layers.BatchNormalization()(conv2)
    conv2 = layers.Activation('relu')(conv2)
    conv2 = layers.Conv2D(32, (3, 3), padding='same', use_bias=False)(conv2)
    conv2 = layers.BatchNormalization()(conv2)
    conv2 = layers.Activation('relu')(conv2)
    pool2 = layers.MaxPooling2D((2, 2))(conv2)

    conv3 = layers.Conv2D(32, (3, 3), padding='same', use_bias=False)(pool2)
    conv3 = layers.BatchNormalization()(conv3)
    conv3 = layers.Activation('relu')(conv3)
    conv3 = layers.Conv2D(32, (3, 3), padding='same', use_bias=False)(conv3)
    conv3 = layers.BatchNormalization()(conv3)
    conv3 = layers.Activation('relu')(conv3)
    pool3 = layers.MaxPooling2D((2, 2))(conv3)

    conv4 = layers.Conv2D(32, (3, 3), padding='same', use_bias=False)(pool3)
    conv4 = layers.BatchNormalization()(conv4)
    conv4 = layers.Activation('relu')(conv4)
    conv4 = layers.Conv2D(32, (3, 3), padding='same', use_bias=False)(conv4)
    conv4 = layers.BatchNormalization()(conv4)
    conv4 = layers.Activation('relu')(conv4)
    pool4 = layers.MaxPooling2D((2, 2))(conv4)
...
for name, build_func in models.items():
    ...
    log_dir = f"all_logs/{name}/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    with tf.summary.create_file_writer(f'all_logs/hparam_tuning/{name}').as_default():
        hp.hparams(hparams)
